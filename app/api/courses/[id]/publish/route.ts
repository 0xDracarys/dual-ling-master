...
export async function POST(
  ...
  try {
    traceLogger.log('info', 'API', 'Publish request received', { courseId });
    ...
    if (userRole !== 'teacher') {
      traceLogger.log('warn', 'API', 'Non-teacher attempted to publish course', {
        userId: teacherId,
        role: userRole,
      });
      traceLogger.endSpan(spanId, 'error', { message: 'Forbidden' });
      return NextResponse.json(
        { success: false, error: 'Only teachers can publish courses' },
        { status: 403 }
      );
    }

    traceLogger.log('info', 'API', 'Attempting to publish course', { courseId });
    await courseService.publishCourse(courseId, teacherId);

    traceLogger.log('success', 'API', 'Course published successfully', { courseId });
    ...
    traceLogger.log('error', 'API', 'Publish failed', {
      error: error.message,
      courseId,
    });
